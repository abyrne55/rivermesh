...
{  
  ...
  // EC 500 EDIT: Display color of message received
  // ########################
  // TODO DISPLAY COLOR! ACTUALLY NVM, don't do that here
  // ########################
		 ...
		 && _tmpMessage.header.hops++ < _max_hops)
	{
	    ...
	    if (_isa_router) {
          ...
          // TODO DISPLAY COLOR! 
          if (tmpMessageLen >= 3) {
            Serial.print(_tmpMessage.data[0]);
            Serial.print(_tmpMessage.data[1]);
            Serial.println(_tmpMessage.data[2]);
          }
          if (tmpMessageLen >= 3) {
          // Assume first byte of message is R, 2nd byte is B, 3rd is G
          //   displayColor(a->data[0], a->data[1], a->data[2]);
            
            red = _tmpMessage.data[0];
            green = _tmpMessage.data[1];
            blue = _tmpMessage.data[2];
            
            analogWrite(13, _gammatable[(int)red]);
            analogWrite(12, _gammatable[(int)green]);
            analogWrite(11, _gammatable[(int)blue]);
